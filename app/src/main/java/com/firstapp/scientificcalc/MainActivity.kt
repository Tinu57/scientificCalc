package com.firstapp.scientificcalc
            ...
            fun evaluate(str: String): Double {
                return object : Any() {
                    ...
                    fun parseExpression(): Double {
                        var x = parseTerm()
                        while (true) {
                            if (eat('+'.digitToInt())) x += parseTerm() // addition
                            else if (eat('-'.digitToInt())) x -= parseTerm() // subtraction
                            else return x
                        }
                    }
                    ...
                    fun parseTerm(): Double {
                        var x = parseFactor()
                        while (true) {
                            if (eat('*'.digitToInt())) x *= parseFactor() // multiplication
                            else if (eat('/'.digitToInt())) x /= parseFactor() // division
                            else return x
                        }
                    }
                    ...
                    fun parseFactor(): Double {
                        ...
                        if (eat('+'.digitToInt())) return parseFactor() // unary plus
                        if (eat('-'.digitToInt())) return -parseFactor() // unary minus
                        ...
                        if (eat('('.digitToInt())) { // parentheses
                            x = parseExpression()
                            eat(')'.code)
                        } else if (ch >= '0'.digitToInt() && ch <= '9'.digitToInt() || ch == '.'.digitToInt()) {
                            // numbers
                            while (ch >= '0'.digitToInt() && ch <= '9'.digitToInt() || ch == '.'.digitToInt()) nextChar()
                            // on below line we are getting sub string from our string using start and pos.
                            x = str.substring(startPos, pos).toDouble()
                        } else if (ch >= 'a'.digitToInt() && ch <= 'z'.digitToInt()) {
                            // on below function we are checking for the operator in our expression.
                            while (ch >= 'a'.digitToInt() && ch <= 'z'.digitToInt()) nextChar()
                        ...
                        if (eat('^'.digitToInt())) x = Math.pow(x, parseFactor()) // exponentiation
                        return x
                    }
